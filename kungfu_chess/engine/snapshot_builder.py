...
def _piece_pixel_position(piece, arbiter: RealTimeArbiter, cell_size_px: int) -> tuple[float, float, float]:
...
def build_snapshot(board: Board, arbiter: RealTimeArbiter, cell_size_px: int,
    ...
    for piece in board.all_pieces():
        if piece.state == PieceState.CAPTURED:
            continue
        px, py, t = _piece_pixel_position(piece, arbiter, cell_size_px)
